...
def load_test_images(test_path: str) -> Dict[str, List[str]]:
    ...
    for label in sorted(['normal', 'anomaly']):
        label_path = os.path.join(test_path, label)
        
        if os.path.isdir(label_path):
            # Sort image files for deterministic ordering
            image_files = sorted([os.path.join(label_path, f) for f in os.listdir(label_path)
                                if f.lower().endswith(('.jpg', '.jpeg', '.png'))])
...
def verify_data_structure(train_path: str, test_path: str) -> Tuple[bool, str]:
    ...
    for category_name in train_classes:
        category_path = os.path.join(train_path, category_name)
        # Sort image files for deterministic ordering
        images = sorted([f for f in os.listdir(category_path)
                        if f.lower().endswith(('.jpg', '.jpeg', '.png'))])
    ...
    for label in sorted(['normal', 'anomaly']):
        ...
        # Sort image files for deterministic ordering    
        test_images = sorted([f for f in os.listdir(label_path)
                            if f.lower().endswith(('.jpg', '.jpeg', '.png'))])
...
def get_class_info(train_path: str) -> Dict[str, int]:
    ...
    for category_name in sorted(os.listdir(train_path)):
        category_path = os.path.join(train_path, category_name)
